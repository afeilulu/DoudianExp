...
public class DiskLruCache {
    ...
    private final int maxCacheItemSize = 512; // 64 item default
    private long maxCacheByteSize = 1024 * 1024 * 32; // 5MB default, set from ImageObjectCache object
    private CompressFormat mCompressFormat = CompressFormat.JPEG;
    private int mCompressQuality = 70; // 70 default
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        if (cacheDir.isDirectory() && cacheDir.canWrite()
                && UIUtils.getUsableSpace(cacheDir) > maxByteSize) {
     ...
     */
    public Bitmap get(String key, int optSize) {
        synchronized (mLinkedHashMap) {
...
//                return BitmapFactory.decodeFile(file);
                ...
                options.inSampleSize = ImageDownloaderLruCache.calculateInSampleSize(options, optSize, optSize);
...
//                    return BitmapFactory.decodeFile(existingFile);
                    /*BitmapFactory.Options options = new BitmapFactory.Options();
                    options.inJustDecodeBounds = true;
                    BitmapFactory.decodeFile(existingFile, options);
                    
                    // Calculate inSampleSize
                    options.inSampleSize = ImageDownloaderLruCache.calculateInSampleSize(options, optSize, optSize);
//                    Log.d(TAG,"optSize = " + optSize);
                    // Decode bitmap with inSampleSize set
                    options.inJustDecodeBounds = false;
                    options.inDither = false;
                    options.inScaled = false;
                    ...
                    try {
    					byte[] bytes = org.apache.commons.io.FileUtils.readFileToByteArray(new File(existingFile));
    					return BitmapFactory.decodeByteArray(bytes,0,bytes.length);
    				} catch (IOException e) {
    					e.printStackTrace();
    				}
    ...
    public boolean containsKey(String key) {
        ...
        final String existingFile = createFilePath(mCacheDir, key);
        if (new File(existingFile).exists()) {
            // File found, addRecent it to the HashMap for future use
            put(key, existingFile);
            return true;
        }
        return false;
...
//                        UIUtils.isExternalStorageRemovable() ?
//                        UIUtils.getExternalCacheDir(context).getPath() :
//                        context.getCacheDir().getPath();
    ...
    private boolean writeBitmapToFile(Bitmap bitmap, String file)
        ...
        try {
            out = new BufferedOutputStream(new FileOutputStream(file), UIUtils.IO_BUFFER_SIZE);
