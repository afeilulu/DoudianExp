package cn.com.xinli.android.doudian.ui.widget.gallery;
/*
...
/**
 * A view that shows items in a center-locked, horizontally scrolling list. <p> The default values for the Gallery assume you will
 * be using {@link R.styleable#Theme_galleryItemBackground} as the background for each View given to the Gallery from the Adapter.
 * If you are not doing this, you may need to adjust some Gallery properties, such as the spacing. <p> Views given to the Gallery
 * should use {@link Gallery_234.LayoutParams} as their layout parameters type. <p/> <p>See the <a
 * href="{@docRoot}resources/tutorials/views/hello-gallery.html">Gallery tutorial</a>.</p>
 */
@Widget
public class Gallery_234 extends AbsSpinner_234 implements GestureDetector.OnGestureListener {

    private static final String TAG = "Gallery";

    private static final boolean localLOGV = false;
    
    private OnKeyListener mOnKeyListener; // added by chen
    ...
    /**
     * When fling runnable runs, it resets this to false. Any method along the path until the end of its run() can set this to
     * true to abort any remaining fling. For example, if we've reached either the leftmost or rightmost item, we will set this to
     * true.
     */
    ...
    public Gallery_234(Context context) {
        this(context, null);
    }

    public Gallery_234(Context context, AttributeSet attrs) {
        this(context, attrs, attr.galleryStyle);
    }

    public Gallery_234(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.Gallery, defStyle, 0);
    ...
    /**
     * Whether or not to callback on any {@link #getOnItemSelectedListener()} while the items are being flinged. If false, only
     * the final selected item will cause the callback. If true, all items between the first and the final will cause callbacks.
     *
    ...
    /**
     * Sets how long the transition animation should run when a child view changes position. Only relevant if animation is turned
     * on.
     *
...
//        t.setAlpha(child == mSelectedChild ? 1.0f : mUnselectedAlpha);
        ...
        // chen
        // selected view is out of gallery and is on top of the selected view in gallery
        // so it is no need to highlight it.
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
         ...
         */
        return new Gallery_234.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    /**
     * Tracks a motion scroll. In reality, this is used to do just about any movement to items (touch scroll, arrow-key scroll,
    ...
    void trackMotionScroll(int deltaX) {
        // AMIR
        if (localLOGV) Log.i(TAG, "trackMotionScroll");
        // end
        ...
        // AMIR
        //Log.w(TAG, "layout:: mRecycledViews size = " + mRecycledViews.size());
        mRecycler.clear();
        // end
    ...
    int getLimitedMotionScrollAmount(boolean motionToLeft, int deltaX) {
        ...
        // AMIR
        // Detect bad (negative) index in order to avoid consuming an exception inside
        // ViewGroup class (wasteful)
        int extremeChildIndex = extremeItemPosition - mFirstPosition;
        View extremeChild = null;
        if (extremeChildIndex >= 0 && extremeChildIndex < getChildCount()) {
            extremeChild = getChildAt(extremeChildIndex);
        }
        // end
    ...
    // AMIR
    private LinkedList<SoftReference<View>> mRecycledViews = new LinkedList<SoftReference<View>>();
    // end
        ...
        if (toLeft) {
            ...
            for (int i = 0; i < numChildren; i++) {
                ...
                if (child.getRight() >= galleryLeft) {
                    ...
                    // AMIR
                    mRecycledViews.add(new SoftReference(child));
                    // end
     ...
     * <p/>
     * We layout rarely, most of the time {@link #trackMotionScroll(int)} takes care of repositioning, adding, and removing
     * children.
     *
     * @param delta Change in the selected position. +1 means the selection is moving to the right, so views are scrolling to the
     *              left. -1 means the selection is moving to the left.
     */
    @Override
    void layout(int delta, boolean animate) {
        // AMIR
        if (localLOGV) Log.i(TAG, "layout");
        // end
        ...
        /*
         * mFirstPosition will be decreased as we addRecent views to the left later
     ...
     * @param x        X-coordintate indicating where this view should be placed. This will either be the left or right edge of
     *                 the view, depending on the fromLeft paramter
     * @param fromLeft Are we posiitoning views based on the left edge? (i.e., building from left to right)?
    ...
    private View makeAndAddView(int position, int offset, int x, boolean fromLeft) {
        ...
        if (child == null && !mRecycledViews.isEmpty()) {
            return getConvertedView(position, offset, x, fromLeft);
        }
    ...
    /**
     * Try finding an unused view inside inner list of recycled views (these views are gathered while we remove not visible views
     * from our layout, see {@link #detachOffScreenChildren(boolean)}). <br/> <br/> In any case, we pass the adapter the recycled
     * view (or null if not found) and return the result
     *
     * @param position Position in the gallery for the view to obtain
     * @param offset   Offset from the selected position
     * @param x        X-coordintate indicating where this view should be placed. This will either be the left or right edge of
     ...
     * @return A view that has been added to the gallery
     */
    private View getConvertedView(int position, int offset, int x, boolean fromLeft) {
        View child = null;

        int count = mRecycledViews.size();
        while (child == null && count-- > 0) {
            child = mRecycledViews.removeFirst().get();
        }
    ...
    private void setUpChild(View child, int offset, int x, boolean fromLeft) {
        ...
        // make some up...
        Gallery_234.LayoutParams lp = (Gallery_234.LayoutParams)
                child.getLayoutParams();
        if (lp == null) {
            lp = (Gallery_234.LayoutParams) generateDefaultLayoutParams();
        }
...
//        return event.dispatch(this, null, null);
    	...
    	// no we need key event. addRecent by chen
    ...
    /**
     * Stop existing fling (she doesn't want you any more!)
     *
     * @param scrollIntoSlots Whether or not to scroll into slots after the scrolling is stopped
     */
    protected void stopFling(boolean scrollIntoSlots) {
        mFlingRunnable.stop(scrollIntoSlots);
    }

    /**
     * Set new scroller for this gallery
     *
     * @param scroller
     */
    protected void setScroller(Scroller_234 scroller) {
        mFlingRunnable.mScroller = scroller;
    }

    /**
     * Responsible for fling behavior. Use {@link #startUsingVelocity(int)} to initiate a fling. Each frame of the fling is
     * handled in {@link #run()}. A FlingRunnable will keep re-posting itself until the fling is done.
     */
    private class FlingRunnable implements Runnable {
        /**
         * Tracks the decay of a fling scroll
         */
        private Scroller_234 mScroller;
        ...
        public FlingRunnable() {
            mScroller = new Scroller_234(getContext());
        }
        ...
        public void run() {
            ...
            final Scroller_234 scroller = mScroller;
