...
import cn.com.xinli.android.doudian.BuildConfig;
...
public class ImageObjectCache {
    ...
    private static final int DEFAULT_DISK_CACHE_SIZE = 1024 * 1024 * 32; // 10MB default
     ...
     */
    public ImageObjectCache(Context context, ImageCacheParams cacheParams) {
     ...
     */
    public ImageObjectCache(Context context, String uniqueName) {
     ...
     */
    public static ImageObjectCache findOrCreateCache(
    ...
    public static ImageObjectCache findOrCreateCache(
        ...
        ImageObjectCache imageCache = (ImageObjectCache) mRetainFragment.getObject();

        // No existing ImageCache, create one and store it in RetainFragment
        if (imageCache == null) {
            imageCache = new ImageObjectCache(activity, cacheParams);
    ...
    private void init(Context context, ImageCacheParams cacheParams) {
        ...
        if (cacheParams.memoryCacheEnabled) {
            mMemoryCache = new LruCache<String, CachedObject>(cacheParams.memCacheSize) {
                /**
                 * Measure item size in bytes rather than units which is more practical for a bitmap
                 * cache
                 */
                @Override
                protected int sizeOf(String key, CachedObject object) {
                	if (object.getBitmap()!=null)
                		return UIUtils.getBitmapSize(object.getBitmap());
                	else
                		return 4;
                }
            };
    ...
    public void addBitmapToCache(String data, CachedObject object) {
        ...
        if (mDiskCache != null && !mDiskCache.containsKey(data)) {
        	if (object.getBitmap()!=null){
        		mDiskCache.put(data, object.getBitmap());
        	}
        }
    }
    
    public void removeItemFromCache(String data) {
    	if (data == null) {
            return;
        }
     ...
     */
    public CachedObject getCachedObjectFromMemCache(String data) {
        if (mMemoryCache != null) {
            final CachedObject object = mMemoryCache.get(data);
            if (object != null) {
...
//                    Log.d(TAG, "Memory cache hit");
     ...
     * @return The bitmap if found in cache, null otherwise
     */
    public Bitmap getBitmapFromDiskCache(String data, int optSize) {
        if (mDiskCache != null) {
            return mDiskCache.get(data,optSize);
        }
        return null;
    }
